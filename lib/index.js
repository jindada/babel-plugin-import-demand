...
var camel2Dash = function camel2Dash(_str) {
  ...
  if (path.node && path.node.source.value === libName) {
    path.node.specifiers.forEach(function (_ref) {
      ...
      if (type === 'ImportSpecifier') {
        path.insertBefore(t.importDeclaration([t.importDefaultSpecifier(t.identifier(imported.name))], t.stringLiteral(libName + '/' + libPath + '/' + (spell ? camel2Dash(imported.name) : imported.name.toLowerCase()))));
        if (cssPath) {
          path.insertBefore(t.importDeclaration([], t.stringLiteral(libName + '/' + libPath + '/' + (spell ? camel2Dash(imported.name) : imported.name.toLowerCase()) + '/' + cssPath)));
        }
      }
    });
    path.remove();
